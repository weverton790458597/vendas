...
async function handleInstagramOauthReturn() {
  ...
  if (connected) {
    ...
    if (currentIgAccounts.length === 0) loadIgConfig();
  }
  if (connected || error) {
...
async function bootstrapSession() {
  try {
    ...
    showBanner("authBanner", "Erro ao carregar a sessão: " + err.message, "error");
    showScreen("auth");
  }
}
...
function updateAutomationIgSelector() {
  ...
  if (currentIgAccounts.length <= 1) {
    ...
    select.innerHTML = currentIgAccounts[0]
      ? '<option value="' + currentIgAccounts[0].id + '">' + (currentIgAccounts[0].instagram_username ? "@" + escapeHtml(currentIgAccounts[0].instagram_username) : "Conta " + currentIgAccounts[0].id.slice(0, 8)) + "</option>"
      : "";
    return;
  }
...
function renderAutomations(rows) {
  ...
  grid.innerHTML = rows.map((row) => {
    ...
    return (
      ...
      '<button class="icon-btn toggle-btn" title="' + (row.ativo ? "Desativar" : "Ativar") + '" data-ativo="' + row.ativo + '">' + (row.ativo ? "🔇" : "🔊") + "</button>" +
...
async function toggleActive(event) {
  ...
  const currentAtivo = btn.getAttribute("data-ativo") === "true";
  try {
...
supabase.auth.onAuthStateChange(async (event, session) => {
  if (event === "SIGNED_OUT") {
    ...
    // Re-bootstrap session after OAuth redirect or token refresh
    if (session && !isInviteOrRecoveryLink()) {
      await bootstrapSession();
    }
  }
});
// O onboarding.js dispacha este evento customizado quando o usuário clica
// "Já aceitei — conectar agora". Como módulos ES não compartilham escopo,
// usamos eventos customizados para comunicação entre arquivos.
document.addEventListener("ig-connect-now", () => {
  const btn = $("#addIgAccountBtn");
  if (!btn) return;
  btn.scrollIntoView({ behavior: "smooth", block: "center" });
  if (btn.disabled) {
    btn.classList.add("ig-tester-pulse");
    setTimeout(() => btn.classList.remove("ig-tester-pulse"), 1600);
    return;
  }
  setTimeout(() => startInstagramConnect(), 350);
});
bootstrapSession().catch((err) => {
  console.error("Fatal: bootstrapSession failed:", err);
});
