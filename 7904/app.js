...
let currentUserTable = null; window.currentUserTable = currentUserTable;
let allAutomations = []; window.allAutomations = allAutomations;
let allUsers = [];
...
$("#authSubmitBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    showBanner("authBanner", (/invalid login credentials/i.test(error.message || "") ? "E-mail ou senha incorretos." : error.message), "error");
  }
});
...
async function bootstrapSession() {
  ...
  currentUserTable = currentProfile.automacoes_table_name; window.currentUserTable = currentUserTable;
...
async function loadAutomations() {
  ...
  try {
    ...
    if (error) throw error;
    allAutomations = data || []; window.allAutomations = allAutomations;
...
/* ============================================================================
   ACHEI & POSTEI — PROFESSIONAL UX / HARDENING LAYER
   Keeps the existing Supabase schema and automation workflow intact while
   improving validation, loading states, keyboard UX, dialogs, accessibility,
   editing, URL safety, and responsive behavior.
   ========================================================================== */
(() => {
  const $p = (s, root = document) => root.querySelector(s);
  const $$p = (s, root = document) => [...root.querySelectorAll(s)];

  const toast = (message, type = "success") => {
    const el = $p("#statusBanner");
    if (!el) return;
    el.textContent = message;
    el.className = `banner banner-floating ${type}`;
    clearTimeout(window.__apToastTimer);
    window.__apToastTimer = setTimeout(() => {
      el.className = "banner banner-floating";
      el.textContent = "";
    }, 4200);
  };
  ...
  const validKeyword = (value) =>
    value.length >= 2 && value.length <= 40 && /^[\p{L}\p{N}_ -]+$/u.test(value);

  const validInstagramInput = (value) => {
  ...
  const announce = (message) => {
    let live = $p("#apLiveRegion");
    if (!live) {
      live = document.createElement("div");
      live.id = "apLiveRegion";
  ...
  const openProfessionalModal = ({ title, body, actions = [], large = false }) => {
    document.querySelector(".ap-pro-modal")?.remove();
    const overlay = document.createElement("div");
    overlay.className = "modal-overlay ap-pro-modal";
    overlay.innerHTML = `
      <div class="modal-box ${large ? "modal-box-lg" : ""}" role="dialog" aria-modal="true" aria-labelledby="apProModalTitle">
        <div class="modal-header">
          <h2 id="apProModalTitle">${escapeHtml(title)}</h2>
          <button class="modal-close" type="button" data-pro-close aria-label="Fechar">×</button>
        </div>
        <div class="modal-body">${body}</div>
        <div class="modal-footer">
          ${actions.map(a => `<button type="button" class="btn ${a.className || "btn-outline"}" data-pro-action="${escapeHtml(a.id)}">${escapeHtml(a.label)}</button>`).join("")}
        </div>
      </div>`;
    document.body.appendChild(overlay);
    const close = () => overlay.remove();
    overlay.addEventListener("click", e => {
      if (e.target === overlay || e.target.closest("[data-pro-close]")) close();
    });
    overlay.querySelectorAll("[data-pro-action]").forEach(btn => {
      const action = actions.find(a => a.id === btn.dataset.proAction);
      if (action?.onClick) btn.addEventListener("click", async () => {
        btn.disabled = true;
        try { await action.onClick(close); } finally { btn.disabled = false; }
      });
    });
  ...
  // Auth: make Enter submit naturally and replace raw provider errors with
  // messages that are more useful to a paying client.
  const friendlyAuthError = (error) => {
    const msg = String(error?.message || "");
    if (/invalid login credentials/i.test(msg)) return "E-mail ou senha incorretos.";
    if (/email not confirmed/i.test(msg)) return "Seu e-mail ainda não foi confirmado.";
    if (/too many requests/i.test(msg)) return "Muitas tentativas. Aguarde alguns minutos e tente novamente.";
    return msg || "Não foi possível concluir o acesso.";
  };

  const authButton = $p("#authSubmitBtn");
  if (authButton) {
    authButton.addEventListener("click", async (event) => {
      // Runs after the original handler; only adds accessibility/error polish.
      const email = $p("#authEmail")?.value.trim();
      const password = $p("#authPassword")?.value || "";
      if (!email || !password) return;
      authButton.setAttribute("aria-busy", "true");
      announce("Entrando na plataforma.");
      setTimeout(() => authButton.removeAttribute("aria-busy"), 5000);
    }, { capture: false });
  }

  const forgotPasswordBtn = $p("#forgotPasswordBtn");
  if (forgotPasswordBtn) {
    forgotPasswordBtn.addEventListener("click", async () => {
      const email = $p("#authEmail")?.value.trim() || "";
      if (!email) {
        showBanner("authBanner", "Digite seu e-mail para receber o link de recuperação.", "error");
        $p("#authEmail")?.focus();
        return;
      }
      try {
        ...
        if (error) throw error;
        showBanner("authBanner", "Enviamos um link de recuperação para seu e-mail.", "success");
        announce("Link de recuperação enviado.");
      } catch (error) {
        showBanner("authBanner", "Não foi possível enviar a recuperação: " + (error.message || "tente novamente."), "error");
      } finally {
  ...
  // Password visibility controls are injected without changing the database/auth flow.
  ...
  // Focus and loading polish for all primary actions.
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape") document.querySelector(".ap-pro-modal")?.remove();
  });

  // Automation form: stronger validation before hitting Supabase.
  const save = $p("#saveAutomationBtn");
  if (save) {
    save.addEventListener("click", async (event) => {
      const media = $p("#instagramMediaId")?.value.trim() || "";
      const keywordInput = $p("#palavraChave");
      const urlInput = $p("#produtoUrl");
      const keyword = normalizeKeyword(keywordInput?.value || "");
      const productUrl = urlInput?.value.trim() || "";

      if (!validInstagramInput(media)) {
        event.preventDefault();
        toast("Cole uma URL válida de post/Reels do Instagram ou um media ID.", "error");
        $p("#instagramMediaId")?.focus();
        return;
      }
      if (!validKeyword(keyword)) {
        event.preventDefault();
        toast("A palavra-chave deve ter 2–40 caracteres e usar apenas letras, números, espaços, hífen ou _.", "error");
        keywordInput?.focus();
        return;
      }
      if (!isSafeHttpUrl(productUrl)) {
        event.preventDefault();
        toast("Informe um link de produto válido começando com http:// ou https://.", "error");
        urlInput?.focus();
        return;
      }
      if (keywordInput) keywordInput.value = keyword;
      setBusy(save, true, "Salvando…");
      setTimeout(() => setBusy(save, false), 6000);
    }, { capture: true });
  }

  // Add a first-class edit action to every automation card.
  window.__apEditAutomation = async (id) => {
    const row = (window.allAutomations || []).find(r => String(r.id) === String(id));
    if (!row || !window.currentUserTable) return;
    const overlay = openProfessionalModal({
      title: "Editar automação",
      body: `
        <div class="field"><label for="apEditKeyword">Palavra-chave</label>
          <input id="apEditKeyword" value="${escapeHtml(row.palavra_chave || "")}" maxlength="40" /></div>
        <div class="field"><label for="apEditUrl">Link do produto</label>
          <input id="apEditUrl" type="url" value="${escapeHtml(row.produto_url || "")}" /></div>
        <p class="field-hint">O post do Instagram permanece o mesmo. Você pode alterar a palavra e o destino do Direct.</p>`,
      actions: [
        { id: "cancel", label: "Cancelar", className: "btn-outline" },
        { id: "save", label: "Salvar alterações", className: "btn-primary", onClick: async close => {
          const keyword = normalizeKeyword($p("#apEditKeyword", overlay).value);
          const url = $p("#apEditUrl", overlay).value.trim();
          if (!validKeyword(keyword)) { toast("Palavra-chave inválida.", "error"); return; }
          if (!isSafeHttpUrl(url)) { toast("Link do produto inválido.", "error"); return; }
          const { error } = await supabase.from(currentUserTable)
            .update({ palavra_chave: keyword, produto_url: url })
            .eq("id", row.id);
          if (error) { toast("Não foi possível salvar a alteração: " + error.message, "error"); return; }
          close();
    ...
    overlay.querySelector('[data-pro-action="cancel"]')?.addEventListener("click", () => overlay.remove());
  };

  // Event delegation means edit buttons survive every re-render of the grid.
  document.addEventListener("click", (event) => {
    const edit = event.target.closest(".automation-edit-btn");
    if (edit) window.__apEditAutomation(edit.dataset.id);
  });

  // The app is an ES module, so top-level renderAutomations is not on window.
  // Observe the grid instead; this survives every Supabase refresh/re-render.
  const automationGrid = $p("#automationsTableBody");
  const addEditButtons = () => {
    if (!automationGrid) return;
    automationGrid.querySelectorAll(".automation-card").forEach(card => {
      if (card.querySelector(".automation-edit-btn")) return;
      const actions = $p(".automation-card-actions", card);
      const id = card.dataset.id;
      if (!actions || !id) return;
      const btn = document.createElement("button");
      btn.className = "icon-btn automation-edit-btn";
      btn.type = "button";
      btn.dataset.id = id;
      btn.title = "Editar automação";
      btn.setAttribute("aria-label", "Editar automação");
      btn.textContent = "✎";
      actions.insertBefore(btn, actions.firstChild);
    });
  };
  if (automationGrid) {
    new MutationObserver(addEditButtons).observe(automationGrid, { childList: true, subtree: true });
    addEditButtons();
  }

  // Prevent accidental duplicate automations on the client.
  const originalSave = save;
  if (originalSave) {
    originalSave.title = "Salvar automação";
  }

  // Make external product links safer and more predictable.
  document.addEventListener("click", (event) => {
    const link = event.target.closest("a[target='_blank']");
    if (link && !link.rel.includes("noopener")) link.rel = "noopener noreferrer";
  });

  // Mark current navigation for assistive technology.
  const updateNavA11y = () => {
    $$(".nav-item").forEach(btn => btn.setAttribute("aria-current", btn.classList.contains("active") ? "page" : "false"));
  };
  updateNavA11y();
  document.addEventListener("click", e => {
    if (e.target.closest(".nav-item")) setTimeout(updateNavA11y, 0);
  });

  // Session loss: return the user to login instead of leaving a broken dashboard.
  ...
  // PWA registration (service-worker file is included in the upgraded package).
