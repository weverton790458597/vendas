...
function goConnectNow() {
  // O app.js gerencia o botão #addIgAccountBtn (texto/disabled/title) a
  // cada renderIgAccounts(). Como os módulos ES não compartilham escopo,
  // disparatamos um evento customizado que o app.js escuta para iniciar
  // a conexão OAuth, evitando depender de .click() em botão potencialmente
  // desabilitado/oculto.
  document.dispatchEvent(new CustomEvent("ig-connect-now"));
}
...
// Also re-export for potential manual cleanup
window.igTesterOnboarding = { stopListening };
