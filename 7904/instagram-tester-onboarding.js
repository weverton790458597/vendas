// instagram-tester-onboarding.js — Redesign 2026
// Lista contas do Instagram solicitadas pelo usuário, cada uma com seu
// status (pendente / aprovado / rejeitado), e mantém o formulário de
// adicionar uma nova conta sempre disponível. Não depende de app.js.
...
function statusBadge(status) {
  if (status === "aprovado")
    return '<span class="badge badge-ativo">Aprovado</span>';
  if (status === "rejeitado")
  ...
  if (status === "aprovado") {
    return `
      <div class="ig-tester-row-note">
        <p>Falta um passo: pelo <strong>navegador</strong> do seu celular (Chrome ou Safari — <strong>não use o app do Instagram</strong>, ele não mostra essa opção):</p>
        <ol>
          ...
          <li>Volte aqui nesta página e clique no botão abaixo (ou em <strong>"Conectar Instagram"</strong>, mais acima) para finalizar.</li>
        </ol>
...
function renderList(requests) {
  ...
  if (!requests || requests.length === 0) {
    list.innerHTML = `
      <div class="ig-tester-empty">
        <svg width="44" height="44" viewBox="0 0 24 24" fill="none"><path d="M21 12.5c0-4.14-.93-7.77-2.86-10.54C17 .86 15.5.37 14 .37V2m-10 14h10" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="1.3" fill="none"/></svg>
        <p>Nenhuma conta do Instagram cadastrada ainda.</p>
      </div>`;
    return;
  }
...
async function loadRequests(userId) {
  ...
  list.innerHTML = '<div class="loading-row">Carregando suas solicitações…</div>';
  ...
  if (error) {
    list.innerHTML = `
      <div class="empty-state">
        Erro ao carregar suas contas: ${escapeHtml(error.message)}
      </div>`;
    return;
  }

  // Não altera o status no banco — apenas esconde da lista o @ que já foi
  // conectado via OAuth, porque não há mais nada pendente de fazer.
...
async function handleSubmit(event) {
  ...
  try {
    ...
    if (error) {
      // Já existe uma solicitação ativa (pendente/aprovada) para este @ —
      // não é um erro grave.
      if (error.code === "23505") {
        setError("Este @ já está cadastrado e em andamento.");
      } else {
        throw error;
      }
    } else if (input) {
...
async function bootstrap() {
  ...
  const list = $("#igTesterRequestsList");
  if (!form && !list) return; // markup não presente nesta página — nada a fazer
...
/* ==========================================================================
   Novo markup esperado em index.html (seção de onboarding):
